...
module uart_regfile_tb;
    ...
    initial begin
        ...
        if (u_dut.status_grp.RX_LVL != 5'h0) begin
        ...
        if (u_dut.status_grp.TX_LVL != 5'h0) begin
        ...
        if (reg_rdata_o[8] != 1'h0) begin
            $display("Reset check for TX_OVRN failed");
            error_cnt++;
        end
        if (reg_rdata_o[13:9] != 5'h0) begin
            $display("Reset check for TX_LVL failed");
            error_cnt++;
        end
        if (reg_rdata_o[14] != 1'h0) begin
        ...
        status_grp.RX_LVL = '1;
        @(negedge clk_i);
        if (reg_rdata_o[6:2] != '1) begin
            $display("update check for RX_LVL failed");
            error_cnt++;
        end else begin
            $display("[%t] Read %0b from RX_LVL", $realtime, reg_rdata_o[6:2]);
        end
        ...
        if (reg_rdata_o[7] != '1) begin
            ...
            $display("[%t] Read %0b from RX_BUSY", $realtime, reg_rdata_o[7]);
        end
        status_grp.TX_OVRN = '1;
        @(negedge clk_i);
        if (reg_rdata_o[8] != '1) begin
            $display("update check for TX_OVRN failed");
            error_cnt++;
        end else begin
            $display("[%t] Read %0b from TX_OVRN", $realtime, reg_rdata_o[8]);
        end
        status_grp.TX_LVL = '1;
        @(negedge clk_i);
        if (reg_rdata_o[13:9] != '1) begin
            $display("update check for TX_LVL failed");
            error_cnt++;
        end else begin
            $display("[%t] Read %0b from TX_LVL", $realtime, reg_rdata_o[13:9]);
        end
        status_grp.TX_BUSY = '1;
        @(negedge clk_i);
        if (reg_rdata_o[14] != '1) begin
            $display("update check for TX_BUSY failed");
            error_cnt++;
        end else begin
            $display("[%t] Read %0b from TX_BUSY", $realtime, reg_rdata_o[14]);
        end
